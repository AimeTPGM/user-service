...
app.config['MONGO_HOST'] = '210.121.158.165'
...
class Login(Resource):
    def post(self):
        ...
        print bcrypt.hashpw(str(data['password']), str(user['password'])) == str(user['password'])
...
class removeRelationship(Resource):
    ...
    def post(self):
        ...
        if not relation:
            return jsonify({"response": "Don't have data"})
        print relation
        mongo.db.relationship.remove({'bossId' : relation['bossId'], 'subordinateId': relation['subordinateId']})
        return Response( json_util.dumps({"response": "remove relation"}, sort_keys=True),mimetype='application/json')

class showRelationship(Resource):
    def get(self, id):
        directBossList = []
        directBossList = mongo.db.relationship.find({'subordinateId': id})
        bosses = []
        for directBoss in directBossList:
            bosses.append(mongo.db.usertestonly.find_one({'_id': ObjectId(directBoss['bossId'])}))
        bosses = serializerList(bosses)
        return Response( json_util.dumps(bosses, sort_keys=True),mimetype='application/json')
...
class getNodeAndLink(Resource):
    ...
    def get(self):
        root = mongo.db.usertestonly.find_one({'firstname':'user1'})
...
api.add_resource(removeRelationship, "/removeRelationship", endpoint="removeRelationship")
api.add_resource(showRelationship, "/user/<string:id>/relationships", endpoint="showRelationship")
